...
G_DEFINE_TYPE(HoloPolkitListener, holo_polkit_listener,
              POLKIT_AGENT_TYPE_LISTENER)

static void finalize(GObject *obj) {
  G_OBJECT_CLASS(holo_polkit_listener_parent_class)->finalize(obj);
}
...
static void auth_free(Auth *auth) {
  g_signal_handlers_disconnect_by_func(auth->cancellable,
                                       ((void *)on_cancelled), 0);
...
static void *do_auth(Auth *auth) {
  char buf[128];
  const char *input = read_input(menu_cmd, buf, 128);
...
static void new_session(Auth *auth) {
  ...
  g_signal_connect(auth->session, "completed", G_CALLBACK(on_session_completed),
                   auth);
}
...
initiate_authentication(PolkitAgentListener *listener, const gchar *action_id,
  ...
  Auth *auth = calloc(sizeof(Auth), 1);
...
static void holo_polkit_listener_class_init(HoloPolkitListenerClass *klass) {
...
static void holo_polkit_listener_init(HoloPolkitListener *self) { (void)self; }

PolkitAgentListener *holo_polkit_listener_new(void) {
  return g_object_new(HOLO_TYPE_POLKIT_LISTENER, NULL);
}
